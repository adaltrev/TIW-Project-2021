...
public class CreateOrder extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (Sid == null || Sid <= 0 || cart == null || cart.size() == 0 || !cart.keySet().contains(Sid)) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST, "No products from this seller in your cart.");
				return;
			}
		} catch (Exception e) {
			e.printStackTrace();
